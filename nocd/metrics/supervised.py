...
def f_evaluate(predicted, groundtruth, beta=1):
    # this f evaluation just become a F1 measure when beta equals 1
    eval_results = list()
    for item_g in groundtruth:
        this_result = list()
        for item_p in predicted:
            numerator = list(set(item_g).intersection(set(item_p)))
            if len(numerator) == 0:
                continue
            precision = len(numerator) / len(item_p)
            recall = len(numerator) / len(item_g)
            F_score = ((1 + (beta ** 2)) * (precision * recall)) / ((beta ** 2) * precision + recall)
            this_result.append(F_score)
        if len(this_result) == 0:
            continue
        eval_results.append(max(this_result))
    return np.mean(eval_results)


def f_evaluate_single(detected_comm, ground_truth_comm, beta=1):
